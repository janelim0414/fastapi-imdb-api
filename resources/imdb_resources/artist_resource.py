...
class ArtistResource(BaseResource):
    ...
    def get(self, primaryName=None, birthYear=None, deathYear=None):
        ...
        if result:
            ...
            for entry in result:
                tmp["data"] = entry
                tmp["links"] = [
                    {"rel": "primaryProfessions", "href": "/api/artists/" + entry["nconst"] + "/primaryProfession"},
                    {"rel": "knownForTitles", "href": "/api/artists/" + entry["nconst"] + "/knownForTitles"},
                    {"rel": "self", "href": "/api/artists/" + entry["nconst"]}
                ]
                final.append(ArtistRsp(**tmp))
    ...
    def post(self, newValues=None):
        ...
        result = ds.insert(self.database, self.collection, newValues)
