...
class MySQLDataService(BaseDataService):
    ...
    def build_update(self, database, collection, predicate, newValues):
        wc, args = MySQLDataService.predicate_to_where_clause_args(predicate)
    ...
    def build_insert(self, database, collection, newValues):
    ...
    def update(self, database, collection, predicate, newValues):
        conn = self.get_connection()

        sql, args = self.build_update(database, collection, predicate, newValues)
        result = self.run_q(sql, args, None, False)
        return result

    def insert(self, database, collection, newValues):
        conn = self.get_connection()

        sql = self.build_insert(database, collection, newValues)
        result = self.run_q(sql, None, None, False)
        if result:
            result = newValues["nconst"]
        return result
